...
public class UnionBySizeCompressingDisjointSetsTests extends BaseTest {
    ...
    // Personal test for correct set id
    @Test
    void findSet_afterUnionOfUnequalSizedTrees_returnsCorrectIds() {
        assertTimeoutPreemptively(Duration.ofSeconds(1), () -> {
            DisjointSets<String> disjointSets = createDisjointSets("Joyce", "Eric", "Keanu");

            disjointSets.union("Joyce", "Eric");
            int helloId = disjointSets.findSet("Joyce");
            disjointSets.union("Joyce", "Keanu");

            int x = disjointSets.findSet("Joyce");
            int y = disjointSets.findSet("Eric");
            int z = disjointSets.findSet("Keanu");
            System.out.println(x + " " + y + " " + z);


            assertThat(disjointSets).findingSets("Joyce", "Eric", "Keanu")
                .containsExactly(helloId, helloId, helloId);
        });
    }
    ...
    void findSet_afterUnionUnequallySized_returnsCorrectIds() {
        assertTimeoutPreemptively(Duration.ofSeconds(1), () -> {
            ...
            disjointSets.union(new IntWrapper(1), new IntWrapper(2));
            int wrapper1Id = disjointSets.findSet(new IntWrapper(0));
